...
import vbll
from vbll_patches import PatchedHetRegression
...
class ParticleSurrogate(nn.Module):
    ...
    def __init__(self,
        ...
        """
        head_type : str  ('standard' or 'het')
            'standard' -> vbll.Regression: aleatoric noise is a fixed,
                learned-but-global parameter per output component. It
                CANNOT vary per event by construction.
            'het' -> vbll.HetRegression: aleatoric noise is produced by a
                second last-layer map M(x) that reads the backbone's hidden
                features per event, so noise CAN vary event-to-event if the
                backbone representation carries any usable signal.
        noise_prior_scale : float  (only used when head_type='het')
            Prior scale for HetRegression's noise-covariance VBLL term.
            Analogous role to wishart_scale for the standard head.
        """
        super().__init__()
        assert head_type in ('standard', 'het'), "head_type must be 'standard' or 'het'"
