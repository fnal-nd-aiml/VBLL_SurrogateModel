...
from cmcrameri import cm as _cmcrameri_cm
...
# cmcrameri's "Managua" — perceptually uniform, colorblind-safe — used for
# all hexbin/density plots instead of the previous mismatched YlOrRd/Blues.
CMAP = _cmcrameri_cm.managua

plt.rcParams.update({
    # Helvetica first; falls back through metric-compatible sans-serif
    # fonts if Helvetica isn't installed on this machine (it's proprietary
    # and often missing on Linux/GPVM systems).
...
def _savefig(fig, name, figures_dir=FIGURES_DIR):
    os.makedirs(figures_dir, exist_ok=True)
    path = os.path.join(figures_dir, name)
    fig.savefig(path, bbox_inches='tight', transparent=True, dpi=350)
    print(f"  Saved → {path}")
    return path
...
def plot_training_curves(history: dict, figures_dir=FIGURES_DIR):
    """
    Four panels:
    ...
    fig.suptitle('Training History', y=1.01)
    ...
    fig.tight_layout()
    _savefig(fig, '1_training_curves.png', figures_dir)
    return fig


# ── 2. Residual distributions ─────────────────────────────────────────────────

def plot_residuals(val_preds: dict, figures_dir=FIGURES_DIR):
    """
    ...
    fig.tight_layout()
    _savefig(fig, '2_residuals.png', figures_dir)
    return fig


# ── 3. Pull distributions ─────────────────────────────────────────────────────

def plot_pulls(val_preds: dict, figures_dir=FIGURES_DIR):
    """
    ...
    axes[1, 0].set_ylabel('Density')
    fig.tight_layout()
    _savefig(fig, '3_pulls.png', figures_dir)
    return fig


# ── 4. Coverage curve ─────────────────────────────────────────────────────────

def plot_coverage(val_preds: dict, figures_dir=FIGURES_DIR):
    """
    ...
    fig.tight_layout()
    _savefig(fig, '4_coverage.png', figures_dir)
    return fig


# ── 5. Uncertainty vs |residual| ──────────────────────────────────────────────

def plot_uncertainty_vs_residual(val_preds: dict, figures_dir=FIGURES_DIR):
    """
    ...
    fig.tight_layout()
    _savefig(fig, '5_uncertainty_vs_residual.png', figures_dir)
    return fig


# ── 6. Truth vs reco scatter ──────────────────────────────────────────────────

def plot_truth_vs_reco(val_preds: dict, figures_dir=FIGURES_DIR):
    """
    ...
    for row, pname in enumerate(PARTICLE_NAMES):
        ...
        for col, comp in enumerate(COMPONENTS):
            ...
            ax.hexbin(true[:, col], pred[:, col], gridsize=40,
                      cmap=CMAP, mincnt=1, linewidths=0.1, alpha=0.7)
    ...
    fig.tight_layout()
    _savefig(fig, '6_truth_vs_reco.png', figures_dir)
    return fig


# ── 7. Outlier uncertainty probe ──────────────────────────────────────────────

def plot_outlier_probe(val_preds: dict, outlier_preds: dict, figures_dir=FIGURES_DIR):
    """
    ...
    fig.tight_layout()
    _savefig(fig, '7_outlier_probe.png', figures_dir)
    return fig


# ── 8. Classification entropy ─────────────────────────────────────────────────

def plot_classification_entropy(val_preds: dict, outlier_preds: dict = None, figures_dir=FIGURES_DIR):
    """
    ...
    fig.tight_layout()
    _savefig(fig, '8_classification_entropy.png', figures_dir)
    return fig


# ── 9. Sigma CV summary ───────────────────────────────────────────────────────

def plot_sigma_summary(val_preds: dict, figures_dir=FIGURES_DIR):
    """
    ...
    fig.tight_layout()
    _savefig(fig, '9_sigma_summary.png', figures_dir)
    return fig


# ── Master call ───────────────────────────────────────────────────────────────

def plot_all(history, val_preds, outlier_preds, normaliser=None, figures_dir=FIGURES_DIR):
    """
    ...
    print("\nGenerating figures...")
    figs = {}
    figs['training']             = plot_training_curves(history, figures_dir=figures_dir)
    figs['residuals']            = plot_residuals(val_preds, figures_dir=figures_dir)
    figs['pulls']                = plot_pulls(val_preds, figures_dir=figures_dir)
    figs['coverage']             = plot_coverage(val_preds, figures_dir=figures_dir)
    figs['uncertainty_vs_resid'] = plot_uncertainty_vs_residual(val_preds, figures_dir=figures_dir)
    figs['truth_vs_reco']        = plot_truth_vs_reco(val_preds, figures_dir=figures_dir)
    figs['outlier_probe']        = plot_outlier_probe(val_preds, outlier_preds, figures_dir=figures_dir)
    figs['cls_entropy']          = plot_classification_entropy(val_preds, outlier_preds, figures_dir=figures_dir)
    figs['sigma_summary']        = plot_sigma_summary(val_preds, figures_dir=figures_dir)
